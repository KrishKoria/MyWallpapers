...
# Find all image files, excluding .git and .thumbnails directories
IMAGE_FILES=$(find . -type f \( -iname "*.png" -o -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.bmp" \) -not -path "./.git/*" -not -path "./.thumbnails/*" -not -path "./.github/*" | sort)
...
while IFS= read -r file; do
  ...
  if [ "$BASENAME" != "$NEW_NAME" ]; then
    ...
    cp "$TEMP_DIR/$NEW_NAME" "${file%/*}/$NEW_NAME"
    rm "$file"
  fi
